...
import Resultats_leaderboard from "../models/resultats_leaderboard.js";
...
/*demander les resultas */
export async function getResults_leaderboard(req, res){
    try {
        const r = await Resultats_leaderboard.find();
        res.json(r)
    } catch (error) {
        res.json({ error })
    }
...
export async function storeResults_leaderboard(req, res){
    try {
        const {username, resultats, essais, points} = req.body;
        if (!username && !resultats) throw new Error('Les Données ne sont pas correctes !');
        const rc = await Resultats_leaderboard.create({username, resultats, essais, points})
            res.json({msg: "Resultats ok !"})
